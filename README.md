...
App页面从全屏模式切换到分屏模式，会经历销毁后重建的过程，所以它的Activity生命周期会从onCreate(）重新走一遍。
...
App页面从全屏模式切换到分屏模式，它的Activity生命周期会经历销毁后重建的过程，如果开发者想保持App页面在分屏前的模样，则需给该页面的activity节点加上以下的属性描述 android:configChanges="screenSize|smallestScreenSize|screenLayout|orientation"

3、分屏在视频播放时的注意事项
对于视频播放页面，需要在它的activity节点加上如下属性描述，android:configChanges="screenSize|smallestScreenSize|screenLayout|orientation"
